...
import { ensureObject, getOptionalEnum, getOptionalString, getRequiredArray, getRequiredString, } from "../utils/validation.js";
const orderInclude = {
    customer: true,
    pickupLocation: true,
    items: {
...
function parseAdminOrderItems(value) {
...
async function prepareOrderItems(items, rentalDays, existingOrder) {
...
function buildInventoryRevertOperations(existingOrder) {
...
function buildInventoryApplyOperations(items) {
...
function buildOrderItemCreates(items) {
    return items.map((item) => ({
        itemType: item.itemType,
        ...(item.lehengaId
            ? {
...
ordersRouter.post("/", asyncHandler(async (request, response) => {
    ...
    const paymentStatus = getOptionalEnum(body, "paymentStatus", Object.values(PaymentStatus)) ?? PaymentStatus.PENDING;
...
ordersRouter.patch("/:id", asyncHandler(async (request, response) => {
    const body = ensureObject(request.body);
    ...
    const rentalStartDate = body.rentalStartDate === undefined
        ? existingOrder.rentalStartDate
        : parseDateValue(getRequiredString(body, "rentalStartDate"), "rentalStartDate");
    const rentalEndDate = body.rentalEndDate === undefined
