...
function parseRentalDateFilter(value, endOfDay = false) {
...
ordersRouter.post("/", asyncHandler(async (request, response) => {
    ...
    const status = getOptionalEnum(body, "status", orderStatusValues) ?? OrderStatus.PENDING;
    const paymentStatus = getOptionalEnum(body, "paymentStatus", paymentStatusValues) ??
...
ordersRouter.patch("/:id", asyncHandler(async (request, response) => {
    ...
    const nextStatus = getOptionalEnum(body, "status", orderStatusValues);
    const nextPaymentStatus = getOptionalEnum(body, "paymentStatus", paymentStatusValues);
...
ordersRouter.post("/:id/refund-deposit", asyncHandler(async (request, response) => {
    ...
    const updatedOrderWithInventory = refund.status === "processed"
