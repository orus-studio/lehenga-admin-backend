...
export function getCatalogImageUrl(key: string) {
...
export function normalizeCatalogImageUrl(imageUrl: string) {
  const cloudFrontBaseUrl = getCloudFrontBaseUrl();
  ...
  try {
    ...
    if (
      (url.hostname === `s3.${env.awsRegion}.amazonaws.com` || url.hostname === "s3.amazonaws.com") &&
